...
test.describe('Boomerang Fu Web', () => {
  test('loads landing page', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('#title-heading')).toBeVisible();
    await expect(page.getByText('Slice and dice your friends with boomerangs')).toBeVisible();
    await expect(page.getByRole('link', { name: 'Play', exact: true }).first()).toBeVisible();
  });

  test('loads play menu from /play', async ({ page }) => {
    await page.goto('/play');
    await expect(page.getByRole('heading', { name: 'BOOMERANG FU', exact: true }).first()).toBeVisible();
